...
// #### Начальная логика: ####
// У меня есть База данных, где каждая строка - это объект, с этим и работаю
// При добавлении, добавляем новую строку - объект
// Индекс берётся на клиенте из массива задач, и задача выцепляется в массике объектов в БД
// При удалении, удаляем объект по какому то индексу
// При какой-то манипуляции обновления объекта, берём объект по индексу из БД, изменяем и заносим обратно в БД
